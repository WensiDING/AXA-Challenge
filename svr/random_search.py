...
num_iter = 20
num_cv = 3
# build a dictionary for all the combination of variables(at least two
# variables)
...
tuned_params = {'maxDepth': sp_randint(3, 15), 'psi': sp_randint(
    200, 300), 'numTree': sp_randint(10, 30), 'Vars': sp_randint(0, count)}
...
with open(outputPath, "a") as myfile:
    class ModeleTree(BaseEstimator):
        ...
        def hs_tree(self, psi, numTree, data):
            ...
            scores[scores > lim] = (
                maxi - scores[scores > lim]) / (maxi - mini)
            # maxi = np.max(scores)
            # mini = np.min(scores)
            # d = maxi-mini
    ...
    tmp = pd.read_csv(labelPath, index_col=False, dtype={
                      'imsi': str}, usecols={'label'})
    ...
    gs = RandomizedSearchCV(ModeleTree(), tuned_params,
